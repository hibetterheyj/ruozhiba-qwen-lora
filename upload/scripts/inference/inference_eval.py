...
用法（仓库根目录为当前工作目录）:
    # 单模型
    python upload/scripts/inference/inference_eval.py \
        --model_path models/Qwen3-4B-Instruct-2507 --tag baseline --gpu 0

    # 批量(扫描 models/merged/ 下所有子目录)
    python upload/scripts/inference/inference_eval.py --model_dir models/merged --gpu 0
...
# upload/scripts/inference: 上两级为 upload/，上三级为仓库根
UPLOAD_ROOT = Path(__file__).resolve().parents[2]
REPO_ROOT = Path(__file__).resolve().parents[3]
DEFAULT_TEST_DATA = UPLOAD_ROOT / "data" / "ruozhiba_cqia_classified_v2.json"
DEFAULT_PROMPTS = UPLOAD_ROOT / "configs" / "prompts.yaml"
