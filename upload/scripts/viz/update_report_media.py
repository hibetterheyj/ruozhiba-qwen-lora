...
def make_training_loss_summary() -> None:
    ...
    for xi, val in zip(x + width / 2, final_vals):
        ax.text(xi, val + 0.01, f'{val:.3f}', ha='center', va='bottom', fontsize=9)
    out = RESULTS_CHARTS / 'bar_train_eval_loss_summary.pdf'
    fig.tight_layout()
    fig.savefig(out, bbox_inches='tight')
    fig.savefig(out.with_suffix('.png'), dpi=180, bbox_inches='tight')
    plt.close(fig)
...
def main() -> None:
    ...
    RESULTS_CHARTS.mkdir(parents=True, exist_ok=True)
    RESULTS_HEATMAPS.mkdir(parents=True, exist_ok=True)
    ...
    copies = [
        ('dataset_source_donut.pdf', RESULTS_CHARTS),
        ('dataset_source_donut.png', RESULTS_CHARTS),
        ('heatmap_all_strict_accuracy.pdf', RESULTS_HEATMAPS),
        ('heatmap_all_strict_accuracy.png', RESULTS_HEATMAPS),
        ('bar_all_vs_last3_delta.pdf', RESULTS_CHARTS),
        ('bar_all_vs_last3_delta.png', RESULTS_CHARTS),
        ('bar_train_eval_loss_summary.pdf', RESULTS_CHARTS),
        ('bar_train_eval_loss_summary.png', RESULTS_CHARTS),
        ('line_training_loss.pdf', RESULTS_CHARTS),
        ('line_training_loss.png', RESULTS_CHARTS),
        ('grid_train_eval_loss.pdf', RESULTS_CHARTS),
        ('grid_train_eval_loss.png', RESULTS_CHARTS),
    ]

    for filename, base in copies:
        copy_if_exists(base / filename, LATEX_MEDIA / filename)
