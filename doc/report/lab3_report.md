...
![](media/fig1_train_eval_loss.png)

*Figure 1. Training loss and evaluation loss curves for all 4 experimental groups (R8/R16 × all/last3). Each subplot shows the training loss (blue) decreasing over steps alongside periodic evaluation loss checkpoints (orange).*

![](media/fig2_eval_loss_trend.png)

*Figure 2. Evaluation loss trend across epochs for all 4 experiments. R16-all reaches its minimum at epoch 5 before rising, indicating mild overfitting, while R8-all continues to decrease through epoch 7.*
...
All 20 SFT models + 1 baseline were evaluated on the 240-sample CQIA test set using greedy decoding (`temperature=0.0`).
...
![](media/fig3_strict_accuracy_trend.png)

*Figure 3. Strict accuracy vs. epoch for all 4 experimental groups. The baseline (dotted gray) is included for reference. All SFT models substantially outperform the baseline, with R16-all peaking at epoch 5.*

![](media/fig4_heatmap_strict_accuracy.png)

*Figure 4. Rank × Epoch heatmap for strict accuracy on the full dataset. R16 consistently outperforms R8, with the darkest cell at R16-E5 (0.613).*

![](media/fig5_confusion_grid.png)

*Figure 5. Normalized confusion matrices for baseline and top-3 SFT models. The SFT models show stronger diagonal concentration, particularly reducing misclassifications of "奇怪提问" (Strange Questions) and "文字游戏" (Wordplay).*

![](media/fig6_per_category_recall.png)

*Figure 6. Per-category recall for the best model (r16_e5). "奇怪提问" achieves the highest recall (0.76), while low-frequency categories like "地狱笑话" and "谐音梗" remain challenging.*

![](media/fig7_baseline_vs_top3.png)

*Figure 7. Grouped bar chart comparing baseline and top-3 models across four key metrics. SFT models show the largest gains in strict accuracy and top-1 accuracy.*

![](media/fig8_all_vs_last3.png)

*Figure 8. Strict accuracy advantage of full-dataset models over last-3-year subset models. All bars are positive, confirming that data volume consistently outweighs data recency for this task.*
