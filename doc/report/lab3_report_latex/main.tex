...
Training was run with two NVIDIA H800 GPUs, each with 80 GB memory. The codebase is organized as a stage-wise pipeline under \texttt{scripts/}, with dedicated subdirectories for crawling, data processing, training, inference, and visualization. The minimal submission package in \texttt{upload/} preserves the core reproducible path while excluding debug-only scripts.
